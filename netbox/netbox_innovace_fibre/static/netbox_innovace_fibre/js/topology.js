...
class DeviceNode {
  constructor(data) {
    ...
    this.color = hashColor(
      this.manufacturer || this.deviceType || String(this.id),
    );
  ...
  draw(ctx) {
    ...
    const sub = [this.manufacturer, this.deviceType]
      .filter(Boolean)
      .join(" · ");
...
class CableEdge {
  ...
  _cp(sc, tc) {
    ...
    return {
      mx: (sc.x + tc.x) / 2 + nx * offset * 2.5,
      my: (sc.y + tc.y) / 2 + ny * offset * 2.5,
      nx,
      ny,
    };
  }
...
class CanvasManager {
  ...
  _onContextMenu(e) {
    ...
    if (edge && this.onContextEdge)
      this.onContextEdge(edge, e.clientX, e.clientY);
  }
  ...
  _onMove(e) {
    if (this._dragging) {
      ...
      let overNode = false;
      this.nodes.forEach((n) => {
        if (n.hitTest(w.x, w.y)) overNode = true;
      });
      const overEdge =
        !overNode &&
        this.edges.some((edge) => edge.hitTest(w.x, w.y, 6 / this.scale));
      this.canvas.style.cursor = overNode
        ? "pointer"
        : overEdge
          ? "context-menu"
          : "default";
...
function _savePositions(nodeMap) {
  ...
  try {
    localStorage.setItem("iff_topo_pos", JSON.stringify(pos));
  } catch {}
}

function _loadPositions() {
  try {
    return JSON.parse(localStorage.getItem("iff_topo_pos") || "{}");
  } catch {
    return {};
  }
}
...
function _clearPositions() {
  try {
    localStorage.removeItem("iff_topo_pos");
  } catch {}
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function createCable(toPort) {
    const body = {
      a_terminations: [
        {
          object_type: connectState.fromPort.object_type,
          object_id: connectState.fromPort.id,
        },
      ],
      b_terminations: [
        { object_type: toPort.object_type, object_id: toPort.id },
      ],
    };
    try {
  ...
  mgr.onContextEdge = (edge, screenX, screenY) => {
    ...
    ctxMenu.style.left =
      Math.min(screenX + 2, window.innerWidth - mw - 8) + "px";
    ctxMenu.style.top =
      Math.min(screenY + 2, window.innerHeight - mh - 8) + "px";
  };

  document
    .getElementById("ctx-trace-ab")
  ...
  async function _traceSignal(startDeviceId, startPort, startSignal = 1) {
    ...
    const queue = [
      { deviceId: startDeviceId, portName: startPort, signal: startSignal },
    ];
        ...
        for (const edge of mgr.edges) {
          if (edge.srcNode.id === deviceId && edge.srcPort === last.to_port) {
            ...
            queue.push({
              ...
              signal: last.to_signal,
            });
          }
          // Also traverse bidirectional exits in the reverse cable direction
          if (
            last.is_bidirectional &&
            edge.tgtNode.id === deviceId &&
            edge.tgtPort === last.to_port
          ) {
            tracedIds.add(edge.id);
            queue.push({
  ...
  function renderDetail(node) {
    ...
    if (ports.length > 0) {
      const isSource =
        connectState.active && connectState.fromNode?.id === node.id;
      const isTarget =
        connectState.active && connectState.fromNode?.id !== node.id;
    ...
    detailBody
      .querySelectorAll(".topo-port-btn:not(.select)")
      .forEach((btn) => {
    ...
    detailBody.querySelectorAll(".topo-port-btn.select").forEach((btn) => {
      btn.addEventListener("click", () =>
        createCable(node.ports[+btn.dataset.idx]),
      );
    });
  }
  ...
  document
    ...
    .addEventListener("click", () => mgr.zoomBy(1.2));
  document
    .getElementById("btn-zoom-out")
    .addEventListener("click", () => mgr.zoomBy(1 / 1.2));
  document
    .getElementById("btn-fit")
    .addEventListener("click", () => mgr.fitView());
  document
    .getElementById("btn-reset-layout")
    .addEventListener("click", () => mgr.resetLayout());
  ...
  async function loadTopology() {
    ...
    try {
      const res = await fetch(
        `/api/plugins/innovace-fibre/topology/?${params}`,
        {
          headers: { "X-CSRFToken": _csrf() },
        },
      );
      ...
      if (!data.nodes || data.nodes.length === 0) {
        ...
        emptyMsg.textContent =
          "No devices with ports found. Add devices in NetBox and reload.";
        return;
      }
      mgr.load(data);
    } catch (err) {
  ...
  document
    ...
    .addEventListener("click", loadTopology);
